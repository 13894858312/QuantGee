...
//                //初始化第i只股票的N日移动均线
//                averageLine = getMaLineFromStockPool(stockPool, beforeDate,stockPool.getEndDate(), yesterday.getCode(), type);
//                //添加到allAverageLine
//                allMaLine.add(averageLine);
//            } else {                                                     //不是第一次加载，直接从allAverageLine中获取
//                averageLine = this.allMaLine.get(i);
//            }
//
//            boolean live = true;                                        //持有期內每天的股票信息必须有 否则不持有该股票
//            for (int j = 0; j < dates.size(); ++j) {
...
//                //计算偏离度，(均值-当天的开盘价)/ 均值
//                double average = 0;
//                if (allMaLine.get(i).get(today) != null) {
//                    average = allMaLine.get(i).get(today).getAverageValue();
//                }
//                double yield = (average - yesterday.getClose()) / average;
//
//                yieldStocks.add(new YieldStock(yesterday.getCode(), yield));
